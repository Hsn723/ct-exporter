...
const (
	defaultParams = "?expand=dns_names&expand=issuer&expand=cert&match_wildcards=true&include_subdomains=true"
)
...
func (c *CertspotterClient) getQueryParams(domain string, position uint64) string {
...
func (c *CertspotterClient) GetIssuances(domain string, position uint64) ([]Issuance, error) {
	queryParams := c.getQueryParams(domain, position)
