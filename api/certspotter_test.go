...
func testGetQueryParams(c CertspotterClient, d string, pos uint64, expected string, t *testing.T) {
	t.Helper()
	actual := c.getQueryParams(d, pos)
	if actual != expected {
...
func TestGetQueryParams(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			domain:   "example.com",
			position: 0,
			expected: "?expand=dns_names&expand=issuer&expand=cert&match_wildcards=true&include_subdomains=true&domain=example.com",
		},
		{
			title:    "testGetQueryParams_WithPosition",
			domain:   "example.com",
			position: 1234567,
			expected: "?expand=dns_names&expand=issuer&expand=cert&match_wildcards=true&include_subdomains=true&domain=example.com&after=1234567",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.title, func(t *testing.T) {
			t.Parallel()
			testGetQueryParams(cc, tc.domain, tc.position, tc.expected, t)
		})
	}
}
...
func TestGetIssuancesEmpty(t *testing.T) {
	...
	expected := []Issuance{}
	actual, err := cc.GetIssuances("example.com", 0)
	if err != nil {
		t.Fatal(err)
	}
